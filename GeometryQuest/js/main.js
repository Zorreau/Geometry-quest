class GameEngine {
  constructor() {
    ...
    this.ownedSkins = new Set(["avatar_gamer", "skin_crosshair_classic", "skin_canvas_dark"]);
    this.equippedSkins = {
      avatar: "avatar_gamer",
...
renderShopItems() {
  ...
  filtered.forEach(skin => {
    ...
    card.className = `p-3.5 rounded-xl border flex flex-col justify-between space-y-2 bg-slate-900/90 ${
      isEquipped ? "border-cyber-accent shadow-[0_0_12px_rgba(0,240,255,0.4)]" : "border-slate-800"
    }`;
    
    card.innerHTML = `
            <div class="flex items-center justify-between">
              <div class="font-bold text-white text-sm flex items-center gap-2">
                <i data-lucide="${skin.iconName || 'sparkles'}" class="w-4 h-4 text-cyber-accent"></i>
                ${skin.name}
              </div>
              <span class="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-950 text-slate-400 border border-slate-800">${skin.gameRef}</span>
            </div>
            <p class="text-xs text-slate-400 font-mono">${skin.desc}</p>
            <div class="flex items-center justify-between pt-2 border-t border-slate-800">
              <span class="text-xs font-mono font-bold text-cyber-gold">${skin.price === 0 ? 'GRATUIT' : skin.price + ' $'}</span>
              <button onclick="game.buyOrEquipSkin('${skin.id}')" class="px-3 py-1.5 rounded-lg text-xs font-bold font-mono transition touch-btn ${
    isEquipped
    ? "bg-cyber-accent text-slate-950 cursor-default"
    : isOwned
    ? "bg-cyber-green/20 text-cyber-green border border-cyber-green"
    : "bg-gradient-to-r from-cyber-accent to-blue-600 text-slate-950"
  }">
                ${isEquipped ? 'ÉQUIPÉ' : isOwned ? 'ÉQUIPER' : 'ACHETER'}
              </button>
            </div>
          `;
...
createNewProfile() {
  ...
  this.profiles[id] = {
    ...
    completedNodes: [],
    visitedWorlds: [],
    ownedSkins: ["avatar_gamer", "skin_crosshair_classic", "skin_canvas_dark"],
    equippedSkins: { avatar: "avatar_gamer", crosshair: "skin_crosshair_classic", canvas: "skin_canvas_dark", ruler: null, square: null, protractor: null }
  };
  nameInput.value = "";
  this.loadProfileState(id);
  this.renderProfilesList();
...
filterShop(category) {
  ...
  document.querySelectorAll(".stab").forEach(btn => {
    btn.classList.remove("active", "bg-cyber-card", "text-cyber-accent", "border-cyber-accent");
    btn.classList.add("bg-slate-900", "text-slate-400", "border-slate-800");
  });
  const activeTab = document.getElementById(`stab-${category}`);
  if (activeTab) {
    activeTab.classList.add("active", "bg-cyber-card", "text-cyber-accent", "border-cyber-accent");
  }
  this.renderShopItems();
}
