Object.assign(GameEngine.prototype, {
  drawWorld1Radar(w, h, gridSize, quest) {
    ...
    if (gd.type === 'spatial3D' || gd.type === 'vector3D' || gd.type === '3D') {
      ...
      ctx.fillText("X (profondeur)", X.px + 10, X.py + 4);
      ctx.fillText("Y (largeur)", Y.px - 20, Y.py + 14);
      ctx.fillText("Z (altitude)", Z.px - 12, Z.py - 8);
    ...
    if (gd.type === 'orthogonal2D' || gd.type === 'orthogonalRelatives') {
      ...
      this.snapPoints.push({ x: originX, y: originY });
      ...
      for (let i = -rangeX; i <= rangeX; i++) {
        ...
        if (gx >= marginLeft && gx <= w - marginRight) {
          ...
          this.snapPoints.push({ x: gx, y: originY });
        }
      }
      
      ctx.textAlign = "right";
      ctx.textBaseline = "middle";
      for (let j = -rangeX; j <= rangeX; j++) {
        if (j === 0) continue;
        ...
        if (gy >= marginTop && gy <= h - marginBottom) {
          ...
          this.snapPoints.push({ x: originX, y: gy });
        }
      }
      ...
      if (!gd.hideTarget && gd.x !== undefined && gd.y !== undefined) {
        ...
        this.snapPoints.push(
          ...
          { x: targetPx, y: originY },
          { x: originX, y: targetPy }
        );
      }
    ...
    if (gd.type === 'cardinalPath' || gd.type === 'dronePath2D') {
      ...
      const isDestinationQuestion = gd.hideTarget || (quest && quest.title && (
        quest.title.toLowerCase().includes("destination") ||
        quest.title.toLowerCase().includes("retrait")
      ));
  ...
  drawWorld2Laser(w, h, quest) {
    ...
    ? (SHOP_SKINS.find(s => s.id === this.equippedSkins.crosshair) || { color: "#00ff66" })
    : { color: "#00ff66" };
    ...
    const bg = ctx.createRadialGradient(cx, cy, 15, cx, cy, Math.max(w, h) / 1.3);
    bg.addColorStop(0, '#091322');
    ...
    if (type === 'shapeIdent') {
      const shape = (gd.shape || 'Triangle').toUpperCase();
      ...
      ctx.beginPath();
      if (shape.includes("TRIANGLE RECTANGLE")) {
        ...
        ctx.fill(); ctx.stroke();
        this.snapPoints.push(p1, p2, p3);
      } else if (shape.includes("CARRÉ") || shape.includes("CARRE")) {
        const p1 = { x: cx - size * 0.7, y: cy - size * 0.7 };
        const p2 = { x: cx + size * 0.7, y: cy - size * 0.7 };
        const p3 = { x: cx + size * 0.7, y: cy + size * 0.7 };
        const p4 = { x: cx - size * 0.7, y: cy + size * 0.7 };
        ctx.rect(p1.x, p1.y, size * 1.4, size * 1.4);
        ctx.fill(); ctx.stroke();
      ...
      const shape = (gd.shape || 'Rectangle').toUpperCase();
      ...
      ctx.beginPath();
      if (shape.includes("CARRÉ") || shape.includes("CARRE")) {
        const p1 = { x: cx - qh, y: cy - qh }, p2 = { x: cx + qh, y: cy - qh }, p3 = { x: cx + qh, y: cy + qh }, p4 = { x: cx - qh, y: cy + qh };
        ctx.rect(p1.x, p1.y, qh * 2, qh * 2);
        ctx.fill(); ctx.stroke();
        ctx.strokeStyle = "#ffe600"; ctx.lineWidth = 2;
        ...
        ctx.strokeRect(p1.x, p4.y - 14, 14, 14);
        this.snapPoints.push(p1, p2, p3, p4, { x: cx, y: cy });
      } else if (shape.includes("LOSANGE")) {
        ...
        ctx.fill(); ctx.stroke();
        this.snapPoints.push(p1, p2, p3, p4, { x: cx, y: cy });
      } else if (shape.includes("PARALLÉLOGRAMME") || shape.includes("PARALLELOGRAMME")) {
        const skew = 25;
      ...
      const shape = (gd.shape || 'Triangle rectangle').toUpperCase();
      ...
      ctx.beginPath();
      if (shape.includes("RECTANGLE ISOCÈLE") || shape.includes("RECTANGLE ISOCELE") || (shape.includes("RECTANGLE") && shape.includes("ISOCÈLE"))) {
        ...
        ctx.strokeRect(p1.x, p1.y - 16, 16, 16);
        this.snapPoints.push(p1, p2, p3);
      } else if (shape.includes("ISOCÈLE") || shape.includes("ISOCELE")) {
      ...
      if (deg === 180) {
        ...
        ctx.strokeStyle = "#ffe600"; ctx.lineWidth = 2; ctx.shadowColor = "#ffe600";
        ctx.beginPath(); ctx.arc(0, 0, 42, 0, -rad, true); ctx.stroke();
        ...
        if (deg === 90) {
          ...
          ctx.fillStyle = "#ffe600"; ctx.font = "bold 15px 'Fira Code', monospace";
          ctx.fillText("?", 50 * Math.cos(-rad / 2), 50 * Math.sin(-rad / 2));
        }
      }
      ...
      const hTri = baseLen * (Math.sin(radA) * Math.sin(radB)) / Math.sin(radSum);
      const xOffset = baseLen * (Math.cos(radA) * Math.sin(radB)) / Math.sin(radSum);
      const p1 = { x: p2.x + xOffset, y: p2.y - hTri };
      ...
      const hypVal = gd ? (gd.hyp || 10) : 10;
      const cosVal = gd ? (gd.cosVal || "0,8") : "0,8";
      
      ctx.fillStyle = "#00f0ff";
      ctx.fillText(`Hypoténuse [BC] = ${hypVal} cm`, cx - 35, cy - 12);
      ctx.fillText(`cos(B) = ${cosVal}`, pB.x - 65, pB.y - 15);
      
      ctx.fillStyle = "#ff0055"; ctx.font = "bold 13px 'Fira Code', monospace";
      ctx.fillText("Côté adjacent [AB] = ?", cx - 50, pA.y + 22);
  ...
  drawWorld3Hitbox(w, h, quest) {
    ...
    if (type === 'alignmentCheck') {
      ...
      const points = [
        ...
        { pos: p3, label: p3Name, color: isAligned ? '#00ff66' : '#ff0055', isTarget: true }
      ];
      ...
      if (isPerp) {
        ...
        ctx.beginPath(); ctx.arc(cx, cy, 30, 0, -angleRad, true); ctx.stroke();
      }
      ...
      const d2 = gd.d2 || '(d2)';
      const d3 = gd.d3 || '(d3)';
      const theoremType = gd.theoremType || 'perpToSame';
      ...
      ctx.strokeStyle = '#00ff66'; ctx.lineWidth = 2; ctx.shadowBlur = 0;
      ctx.strokeRect(perpX, y2 - 14, 14, 14);
      
      if (theoremType === 'perpToSame') {
        ctx.strokeRect(perpX, y1, 14, 14);
      } else {
      ...
      ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 2.5; ctx.shadowColor = '#ffd700';
      ...
      ctx.strokeStyle = '#00ff66'; ctx.lineWidth = 2; ctx.shadowBlur = 0;
      ctx.strokeRect(footPos.x, footPos.y - 14, 14, 14);
      ...
      if (lineType === 'Hauteur') {
        ...
        ctx.strokeStyle = '#ffe600'; ctx.lineWidth = 1.8; ctx.shadowBlur = 0;
        ctx.strokeRect(H.x, H.y - 12, 12, 12);
        this.snapPoints.push(H);
      } else if (lineType === 'Médiatrice') {
        ctx.beginPath(); ctx.moveTo(M.x, M.y - 120); ctx.lineTo(M.x, M.y + 25); ctx.stroke();
        ctx.strokeStyle = '#ffe600'; ctx.lineWidth = 1.8; ctx.shadowBlur = 0;
        ctx.strokeRect(M.x, M.y - 12, 12, 12);
      ...
      const pulseRadius = 60 + Math.sin(time * 3) * 15;
      ctx.strokeStyle = '#00f0ff'; ctx.lineWidth = 2; ctx.shadowColor = '#00f0ff'; ctx.shadowBlur = 12;
  ...
  drawWorld4Mirror(w, h, quest) {
    ...
    ctx.clearRect(0, 0, w, h);
    const bg = ctx.createLinearGradient(0, 0, w, h);
    bg.addColorStop(0, '#0d0614');
    bg.addColorStop(1, '#1a0826');
    ctx.fillStyle = bg;
    ctx.fillRect(0, 0, w, h);
    ...
    if (type === 'axialGrid' || type === 'axialGridError') {
      ...
      const isDistanceQuestion = quest && quest.title && quest.title.includes("Position Symétrique");
      ...
      if (type === 'axialGridError') {
        ...
        ctx.setLineDash([]);
        this.snapPoints.push({ x: pSymX, y: pSymX });
      }
    }
  ...
  drawWorld5Solids(w, h, quest) {
    ...
    if (type === 'solid3D') {
      ...
      ctx.setLineDash([5, 5]); ctx.strokeStyle = "rgba(0, 240, 255, 0.45)"; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(O.px, O.py); ctx.lineTo(Y.px, Y.py);
      ctx.moveTo(O.px, O.py); ctx.lineTo(X.px, X.py);
      ctx.moveTo(O.px, O.py); ctx.lineTo(Z.px, Z.py); ctx.stroke();
      ...
      ctx.strokeStyle = "#ff0055"; ctx.lineWidth = 3.5; ctx.shadowColor = "#ff0055"; ctx.shadowBlur = 12;
      ctx.beginPath(); ctx.moveTo(X.px, X.py); ctx.lineTo(XZ.px, XZ.py); ctx.stroke();
      ...
      netGrid.forEach(f => {
        ...
        ctx.setLineDash([4, 4]); ctx.strokeStyle = "#ffd700"; ctx.lineWidth = 1.5;
        ctx.strokeRect(fx + 2, fy + 2, squareSize - 4, squareSize - 4);
        ctx.setLineDash([]); ctx.strokeStyle = "#00f0ff"; ctx.lineWidth = 2.5;
        
        this.snapPoints.push(
          { x: fx, y: fy }, { x: fx + squareSize, y: fy },
          { x: fx + squareSize, y: fy + squareSize }, { x: fx, y: fy + squareSize },
          { x: fx + squareSize / 2, y: fy + squareSize / 2 }
        );
      });
    }
      ...
      if (type === 'cylinderVol') {
        ...
        ctx.fillText(`R = ${gd.R || '?'} m`, cx + rx / 2 - 15, cy - ch / 2 - 8);
        ...
        ctx.fillText("V = ?", cx - 20, cy + ch / 2 + 30);
        ...
        ctx.setLineDash([4, 4]); ctx.strokeStyle = "rgba(0, 240, 255, 0.4)"; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(O.px, O.py); ctx.lineTo(Y.px, Y.py);
        ctx.moveTo(O.px, O.py); ctx.lineTo(X.px, X.py);
        ctx.moveTo(O.px, O.py); ctx.lineTo(Z.px, Z.py); ctx.stroke();
  ...
  drawOverlayTool(w, h) {
    ...
    if (this.activeTool === "ruler") {
      const skin = SHOP_SKINS.find(s => s.id === this.equippedSkins.ruler) || { renderStyle: "assassin_blade" };
      const rWidth = 260, rHeight = 44;
      ...
      if (skin.renderStyle === "dragstrip") {
        ...
        ctx.fillStyle = "#ffffff";
        for (let d = 0; d < rWidth; d += 20) {
          ctx.fillRect(d, 0, 10, 6);
          ctx.fillRect(d + 10, 6, 10, 6);
        }
        
        ctx.strokeStyle = "#ff9900"; ctx.lineWidth = 1.5;
        for (let i = 10; i < rWidth; i += 10) {
          const hTick = i % 50 === 0 ? 16 : (i % 20 === 0 ? 12 : 7);
          ctx.beginPath(); ctx.moveTo(i, rHeight); ctx.lineTo(i, rHeight - hTick); ctx.stroke();
        }
        ctx.fillStyle = "#ff9900"; ctx.font = "bold 10px 'Fira Code', monospace";
        ctx.fillText("MOTORFEST_DRAGSTRIP // NITRO", 15, 22);
        
      } else {
        const grad = ctx.createLinearGradient(0, 0, rWidth, 0);
        grad.addColorStop(0, "rgba(15, 23, 42, 0.95)");
        grad.addColorStop(0.7, "rgba(30, 41, 59, 0.9)");
        ...
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.lineTo(rWidth - 30, 0);
        ctx.lineTo(rWidth, rHeight / 2);
        ctx.lineTo(rWidth - 30, rHeight);
        ctx.lineTo(0, rHeight);
        ctx.closePath();
        ctx.fill(); ctx.stroke();
        
        ctx.strokeStyle = "#00f0ff"; ctx.lineWidth = 1.5;
        for (let i = 10; i < rWidth - 35; i += 10) {
          const hTick = i % 50 === 0 ? 14 : (i % 20 === 0 ? 10 : 6);
          ctx.beginPath(); ctx.moveTo(i, 0); ctx.lineTo(i, hTick); ctx.stroke();
        }
        ctx.fillStyle = "#00f0ff"; ctx.font = "bold 10px 'Fira Code', monospace";
        ctx.fillText("ASSASSIN_BLADE // 250mm", 15, rHeight - 10);
      }
    }
    
    // 2. ÉQUERRE LASER (SQUARE)
    else if (this.activeTool === "square") {
      const skin = SHOP_SKINS.find(s => s.id === this.equippedSkins.square) || { renderStyle: "redstone_block" };
      const sSize = 150;
      ...
      if (skin.renderStyle === "cyber_katana") {
        ...
        ctx.fillStyle = "#ff0055"; ctx.font = "bold 10px 'Fira Code', monospace";
        ctx.fillText("MONO_KATANA_90°", 12, -25);
        ...
        ctx.fillStyle = "#ffffff"; ctx.font = "bold 10px 'Fira Code', monospace";
        ctx.fillText("REDSTONE_BLOCK_90°", 15, -15);
      }
    }
    
    // 3. RAPPORTEUR HOLO (PROTRACTOR)
    else if (this.activeTool === "protractor") {
      const skin = SHOP_SKINS.find(s => s.id === this.equippedSkins.protractor) || { renderStyle: "vice_speedo" };
      const radius = 100;
      
      if (skin.renderStyle === "vice_speedo") {
        ctx.fillStyle = "rgba(31, 5, 41, 0.85)";
        ...
        for (let a = 0; a <= 180; a += 10) {
          ...
          const len = a % 30 === 0 ? 14 : 8;
          const x1 = Math.cos(Math.PI + rad) * radius;
          const y1 = Math.sin(Math.PI + rad) * radius;
          const x2 = Math.cos(Math.PI + rad) * (radius - len);
          const y2 = Math.sin(Math.PI + rad) * (radius - len);
          ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); ctx.stroke();
        }
        
        ctx.fillStyle = "#00f0ff"; ctx.font = "bold 10px 'Fira Code', monospace";
        ctx.fillText("VICE_CITY_180°", -35, -20);
